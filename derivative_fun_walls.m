...
%% =========== u ==========
    ...
    st.u(2:end-1, :) = ...
        mesh.g * ...
            ...
            diff(h_per(2:end-1, :), 1, 2) / mesh.dx);
%% =========== w =============
    d = diff([h_per, state.h(:, 1)], 2, 2) / (2 * mesh.dx);
    st.w = zeros(mesh.Ny + 1, mesh.Nx);
    st.w(2:end-1, :) = mesh.g * ...
        (((d(1:end-1, :) + d(2:end, :)) / 2) .* grid.sw(2:end-1, :) - ...
        (-diff(state.h, 1, 1) / mesh.dy) .* (1 + grid.sw(2:end-1, :).^2) .* grid.etaY_w(2:end-1, :));
...
%% ========== h =============
    ...
    st.h = -mesh.H * grid.etaY_h .* ...
               (diff(state.w, 1, 1) / mesh.dy + ...
                diff([state.u, state.u(:, 1)]./ [grid.etaY_u, grid.etaY_u(:, 1)], 1, 2) / mesh.dx);
end
